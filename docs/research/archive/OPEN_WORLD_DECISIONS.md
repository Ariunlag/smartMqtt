...
- Duplicate identity is represented separately from pair-level duplicate review history. `KEEP_BOTH` is an identity decision only and never creates semantic negative feedback or prevents the two topics from sharing a semantic class.
- A fresh confirmed duplicate records an explicit alias-to-canonical root in PostgreSQL. Existing pre-migration confirmed rows remain terminal but diagnostically `legacy/unresolved`; the system does not guess their missing unsubscribe target from subscription state.
- Canonicalization is an application workflow using the shared `SemanticApplication`. It preflights human membership and negative-constraint conflicts, reconciles runtime/UNKNOWN/candidate/membership/constraint/prototype state, advances the semantic context generation, and requests bounded rediscovery after authoritative state commits.
- Conflicting human-confirmed classes reject duplicate canonicalization. Compatible membership and constraints transfer to the canonical topic and affected six-view prototypes count the logical stream once.
- User class writes are deterministically canonicalized. Tag-group results count and return distinct canonical topics. Historical Influx measurements and retained Qdrant vectors keep their original topic names; this phase does not invent a historical time-series merge.
- The MQTT identity gate runs before topic detection, Influx writing, WebSocket publication, and semantic processing. Messages for confirmed aliases are diagnosed and dropped. Manual subscription of an alias returns a conflict naming its canonical topic.
- Tag-group centroids still reflect historical unique tag values rather than canonical stream membership. Correcting that quality bias remains Phase 3 work; active topic counts are canonical now.
